...
public class DatePickerPlugin extends CordovaPlugin {
	...
	public synchronized void show(final JSONArray data, final CallbackContext callbackContext) {
		...
		try {
			...
			Log.d(pluginName, "Processing min and max dates...");

            if(!obj.isNull("minDate")) {
                String minDateString = obj.getString("minDate");
                if(!minDateString.equals("0")) {
                    try {
                        Date date = format.parse(minDateString);
                        Log.d(pluginName, "minDate: " + date.toString());
                        minDateLong = date.getTime();
                    } catch (Exception e) {
                        e.printStackTrace();
                     }
                 }
                ...
                Log.d(pluginName, "No min date ");
            }

            if(!obj.isNull("maxDate")) {
                String maxDateString = obj.getString("maxDate");
                if(!maxDateString.equals("0")) {
                    try {
                        Date date = format.parse(maxDateString);
                        Log.d(pluginName, "maxDate: "+ date.toString());
                        maxDateLong = date.getTime();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
